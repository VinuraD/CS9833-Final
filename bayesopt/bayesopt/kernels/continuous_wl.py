...
class ContinuousWeisfeilerLehman:
    ...
    def parse_input(self, X, return_label=False, train_mode=True):
        ...
        if len(node_features) == 0 or total_nodes == 0:
            # No nodes to process; return a small placeholder to keep downstream logic alive
            return np.zeros((len(node_features), 1))

...
from .cf_gnn_utils.explainer import Explainer
from .cf_gnn_utils.utils import build_counterfactual_graph_gc, get_optimizer
# from omegaconf import DictConfig
from torch_geometric.data import Data, Batch
import time
from .cf_gnn_utils.edge_perturber import EdgePerturber   
...
class CFExplainer(Explainer):
    ...
    def __init__(self, datainfo):
        super().__init__(datainfo=datainfo)   
        
        self.set_reproducibility()
        
    def explain(self, graph: Batch, oracle: nn.Module) -> Data:
        """
        ...
        self.edge_perturber = EdgePerturber(num_classes=self.datainfo.num_classes, 
                                            model=oracle, 
                                            graph=graph)
        ...
        self.optimizer = optim.Adam(self.edge_perturber.parameters(), lr=0.01)
        ...
        for _ in range(500): #self.cfg.optimizer.num_epochs
            new_sample = self.train(graph, oracle)

            if time.time() - start_time > 500: #self.cfg.timeout:
                break
    ...
    def train(self, graph: Batch, oracle: nn.Module) -> Data:
        """
        ...
        if y_non_differentiable == graph.targets and loss.item() < self.best_loss:
            ...
            counterfactual = build_counterfactual_graph_gc(graph, oracle, output_actual)

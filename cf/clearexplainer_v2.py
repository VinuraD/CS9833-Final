...
sys.path.append('../')
from dataset_utils import load_tud_dataset, load_dataset_splits, dataset_root
...
def _train_clear_prediction_model(data_bundle: dict,
    ...
    def run_epoch(loader, train: bool):
        ...
        for batch in loader:
            ...
            num_nodes = torch.as_tensor(batch['num_node_real'], dtype=torch.long, device=device)

            if train:
                optimizer.zero_grad()

            logits = model(features, adj, num_nodes=num_nodes)['y_pred']
...
def train_clear_model(dataset_name: str,
    ...
    if not os.path.exists(prediction_weights):
        ...
        test_acc = _train_clear_prediction_model(data_bundle, dataset_name, prediction_dir, device)
        print(f'Prediction model test accuracy: {test_acc:.4f}')
...
def compute_loss(params):
    ...
    pred_device = next(pred_model.parameters()).device
    features_reconst_pred = features_reconst.to(pred_device)
    adj_reconst_pred = adj_reconst.to(pred_device)
    y_cf_pred = y_cf.to(pred_device)
    num_nodes_pred = num_nodes.to(pred_device) if num_nodes is not None else None

    y_pred = pred_model(features_reconst_pred, adj_reconst_pred, num_nodes=num_nodes_pred)['y_pred']  # n x num_class
    loss_cfe = F.nll_loss(F.log_softmax(y_pred, dim=-1), y_cf_pred.view(-1).long())
    loss_cfe = loss_cfe.to(features_reconst.device)
...
def train(params):
    ...
    for epoch in range(epochs + 1):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            num_nodes_real = torch.as_tensor(data['num_node_real'], dtype=torch.long, device=device)
...
def test(params):
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        adj_reconst_binary = torch.bernoulli(adj_reconst)
        pred_device = next(pred_model.parameters()).device
        features_reconst_cpu = features_reconst.to(pred_device)
        adj_reconst_binary_cpu = adj_reconst_binary.to(pred_device)
        features_cpu = features.to(pred_device)
        adj_cpu = adj.to(pred_device)
        num_nodes_cpu = num_nodes_real.to(pred_device)
        y_cf_pred = pred_model(features_reconst_cpu, adj_reconst_binary_cpu, num_nodes=num_nodes_cpu)['y_pred']
        y_pred = pred_model(features_cpu, adj_cpu, num_nodes=num_nodes_cpu)['y_pred']
        y_cf_pred = y_cf_pred.to(device)
...
def run_clear(args, exp_type, save_model=False, data_bundle: Optional[dict] = None):
    ...
    if not os.path.exists(prediction_path):
        ...
        # pred_device = torch.device("cpu")
        test_acc = _train_clear_prediction_model(data_load, args.dataset, prediction_dir, device)
        print(f'Prediction model test accuracy: {test_acc:.4f}')
    ...
    if pred_model_type == 'GIN':
        pred_model = models.Graph_pred_model(input_dim, hidden_dim, output_dim, max_num_nodes, args.dataset)
    else:
        raise ValueError(f'Unsupported prediction model type for CLEAR: {pred_model_type}')
    pred_model.load_state_dict(torch.load(prediction_path, map_location=pred_device))
    pred_model = pred_model.to(pred_device)
    pred_model.eval()

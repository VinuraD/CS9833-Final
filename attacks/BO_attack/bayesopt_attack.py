...
class BayesOptAttack(BaseAttack):
    ...
    def attack(self, graph: dgl.DGLGraph, label: torch.tensor, budget, max_queries: int):
        ...
        while i < max_queries:
            ...
            with torch.no_grad():
                ...
                self.loss_history += losses_cpu.numpy().tolist()

                if self.verbose:
                    print(f'Iteration {i}. Loss: {losses_cpu.numpy()}.')

            dfs.append(self.construct_dataframe(losses_cpu, preds_cpu, label.squeeze(), i + 1))
            ...
            if (self.target_class is None and np.sum(correct_predictions(preds_np, labels_np)) < len(perturbed_graphs)) \
                    or (self.target_class is not None and (np.argmax(preds_np, axis=1) == self.target_class).any()):
                print('Attack succeeded!')
                if self.target_class is None:
                    comps = correct_predictions(preds_np, labels_np)
                    ...
                    for i, pred in enumerate(preds_np):
                        if np.argmax(pred) == self.target_class:
                            adv_example = perturbed_graphs[i]
                            break
                break
            ...
            if np.max(losses_cpu.numpy()) > best_loss:
                n_fail = 0
                best_loss = torch.max(losses_cpu).detach().numpy()
            else:
    ...
    def suggest(self, base_graph: dgl.DGLGraph, n_edit: int, prohibited_edges: list = None):
        ...
        if len(candidate_graphs) == 0:
            # Fallback to a random proposal to keep the BO loop alive
            if self.mode == 'rewire':
                candidate_samples = [random_sample_rewire_swap(base_graph, max(1, n_edit),
                                                               rewire_only=not is_edge_weighted,
                                                               n_hop=self.n_hop_constraint,
                                                               preserve_disconnected_components=self.preserve_disconnected_components)]
            else:
                candidate_samples = [random_sample_flip(base_graph, max(1, n_edit),
    ...
    @staticmethod
    def construct_dataframe(losses: torch.Tensor, predictions: torch.tensor, label: torch.tensor, queries: int) \
            -> pd.DataFrame:
        """
        ...
        Converts tensors to CPU/NumPy to avoid device-related crashes.
        """
        losses_np = losses.detach().cpu().numpy()
        preds_np = predictions.detach().cpu().numpy()
        label_np = label.detach().cpu().numpy() if torch.is_tensor(label) else np.asarray(label)
        labels = np.tile(label_np, len(preds_np))
        df = pd.DataFrame({'losses': losses_np,
                           'correct_prediction': correct_predictions(preds_np, labels),
                           'queries': queries})
        return df

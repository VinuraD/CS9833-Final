...
class PR_Attack:
    ...
    def projection(self,graph):
        ...
        discretized_perturbations = torch.zeros_like(perturbations)
        flat_perturbations = perturbations.view(-1)
        _, topk_indices = torch.topk(flat_perturbations, self.k)
        discretized_perturbations.view(-1)[topk_indices] = 1.0
    ...
    def train(self, test_graphs):
        ...
        for e in range(self.epochs):
            ...
            adj=to_dense_adj(edge_index=graph.edge_index)[0] 
            perturbations = self.ranking(adj,mask)
            perturbed_graph = adj + perturbations
            att_edge_index,att_edge_weight = dense_to_sparse(perturbed_graph)
            att_graph = copy.deepcopy(graph)
            att_graph.edge_index = att_edge_index
            att_prediction = self.model.predict_vector(att_graph,att_edge_weight,device=self.device)
            # print('att prediction ', att_prediction)

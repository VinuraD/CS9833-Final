...
parser.add_argument('--rewatt_action_pct', type=float, default=0.03, help='Fraction of edges to consider for ReWatt rewiring steps (default: 0.03)')
...
parser.add_argument('--rewatt_negative_reward', type=float, default=-0.5, help='Negative reward to use when ReWatt fails to flip prediction (default: -0.5)')
...
parser.add_argument('--use_target', action='store_true', help='Load models trained on targeted counterfactual pairs (cf_lm_target dir)')
parser.add_argument('--use_base_conf', action='store_true', help='Load models trained on base-confidence-filtered targeted CFs (cf_lm_target_conf dir)')
...
if __name__ == '__main__':
    ...
    flag_count = sum(int(v) for v in (args.use_base_conf, args.use_target, args.use_pairs))
    if flag_count > 1:
        raise ValueError("Choose only one of --use_base_conf, --use_target, or --use_pairs when loading models.")

    if cf_pct != 0:
        if args.use_base_conf:
            model_path = os.path.join(model_path, 'cf_lm_target_conf', args.cf_type)
        elif args.use_target:
            model_path = os.path.join(model_path, 'cf_lm_target', args.cf_type)
        elif args.use_pairs:
            model_path = os.path.join(model_path, 'cf_lm', args.cf_type)
        else:
            model_path = os.path.join(model_path, 'cf', args.cf_type)
    elif flag_count:
        # When no counterfactual percentage is requested, keep the vanilla model even if cf flags are passed.
        print('cf_pct is 0; loading vanilla model despite counterfactual flags.')
    if model_name=='GCN':
    ...
    if attack == 'BB':
        ...
        # num_test_cls_1 = sum(1 for i in range(num_test) if check_class(test_dataset[i]))
        denom = max(1, num_test - no_need_count)
        print('{} instances don\'t need to be attacked'.format(no_need_count))
        print("successs ratio:{}".format((success_count / denom*100)))
        '''
        ...
        for i in range(num_test):
            ...
            space = max(1, num_nodes * (num_nodes - 1) / 2)
            if y0 == y1 and y0 == 1:
                target_label = (y0 + 1) % output_dim
                time_start = time()
                adv_data, success, steps = attacker.attack(
                    x0, targeted=True, target_label=target_label, initial_pred=y1
                )
                time_end = time()
                attack_time.append(time_end - time_start)
        ...
        print(f'ReWatt targeted attack success rate: {success_count}/{total_attempts} = {success_ratio:.2f}%')
        if success_count > 0:
            print(f'ReWatt targeted average perturbation: {np.mean(success_perturbs):.2f}')
            print(f'ReWatt targeted average perturbation ratio: {np.mean(success_ratios) * 100:.2f}%')
            print(f'ReWatt targeted average time: {np.mean(success_times):.4f}s')
            print(f'ReWatt targeted average steps: {np.mean(success_steps):.2f}')
            if num_add_edge and num_delete_edge:
                print(f'ReWatt targeted edges added (mean): {np.mean(num_add_edge):.2f}, edges deleted (mean): {np.mean(num_delete_edge):.2f}')

        defense_dir = os.path.join('./attacks', 'defense_rewatt')
        os.makedirs(defense_dir, exist_ok=True)
        defense_prefix = os.path.join(defense_dir, f'{dataset_name}_{model_name}_ReWatt_target_')
        torch.save(detect_test_normal, defense_prefix + 'test_normal.pt')
        torch.save(detect_test_advers, defense_prefix + 'test_advers.pt')
    elif attack == 'BO':
        try:
        ...
        for i in range(num_test):
            ...
            if y0 == y1 and y0.item() == 1:
                ...
                print('instance {} is wrongly classified or not class 1, No Need to Attack'.format(i))
                no_need_count += 1
                perturbation.append(0)
                perturbation_ratio.append(0)
                attack_time.append(0)
